...
export function useConviteBanca({
	...
	onConviteEnviado,
	onClose,
}: UseConviteBancaParams) {
	// Estados de dados
	const [docentesBanca, setDocentesBanca] = useState<Docente[]>([]);
	...
	async function carregarDocentesBanca() {
		try {
			...
			const docentes =
				await conviteBancaService.getDocentesBancaPorCurso(
					Number(idCurso),
				);
	...
	function handleChangeOrientadores(event: {
		...
		// Limitar seleção
		if (valueArray.length <= limiteSelecao) {
			setOrientadoresSelecionados(valueArray);
	...
	function getDocenteBancaNome(codigo: string) {
	...
	function isDocenteDisabled(docente: Docente) {
	...
	function getDocenteSecondaryText(docente: Docente) {
		const ehOrientador = conviteBancaController.verificarEhOrientador(
			docente.codigo,
			conviteOrientacao,
		);
