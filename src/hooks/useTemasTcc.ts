...
export function useTemasTcc(isOrientadorView = false) {
	...
	const [temaDelete, setTemaDelete] = useState<number | null>(null);
	...
	async function getCursosOrientador() {
		try {
			...
			const cursoUnico =
				temasTccController.devPreSelecionarCurso(
					cursosExtraidos as { id: number }[],
				);
			if (cursoUnico) {
	...
	async function getAreasTccPorDocente(codigoDocente: string) {
		try {
			const areas =
	...
	async function getTemasPorCurso() {
		try {
			const temasData =
				await temasTccService.getTemasPorCurso(Number(cursoSelecionado));
			setTemas(temasData);
		} catch (error) {
			console.log(
	...
	function handleDelete(row: TemaTcc) {
		setTemaDelete(row.id);
		setOpenDialog(true);
	}

	function handleOpenVagasModal(tema: TemaTcc) {
	...
	async function handleUpdateVagas() {
		try {
			...
			setMessageText(
				error instanceof Error ? error.message : "Falha ao atualizar vagas da oferta!",
			);
	...
	function handleCursoChange(e: { target: { value: string | number } }) {
	...
	function handleNovaAreaChange(e: ChangeEvent<HTMLInputElement>) {
	...
	async function handleCreateArea() {
		try {
			...
			setMessageText(
				error instanceof Error ? error.message : "Falha ao criar área TCC!",
			);
			setMessageSeverity("error");
		} finally {
			setOpenMessage(true);
	...
	async function handleAddTema() {
		try {
			...
			setMessageText(
				error instanceof Error ? error.message : "Falha ao gravar tema TCC!",
			);
			setMessageSeverity("error");
		} finally {
			setOpenMessage(true);
	...
	async function handleDeleteClick() {
		try {
			...
			setMessageText(
				error instanceof Error ? error.message : "Falha ao remover tema TCC!",
			);
			setMessageSeverity("error");
		} finally {
			setTemaDelete(null);
	...
	async function handleToggleAtivo(tema: TemaTcc) {
		try {
			...
			setMessageText(
				error instanceof Error ? error.message : "Falha ao alterar status do tema!",
			);
			setMessageSeverity("error");
		} finally {
			setOpenMessage(true);
