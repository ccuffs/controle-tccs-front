...
interface FiltrosPesquisaProps {
	...
	setCursoSelecionado: (value: string | number) => void;
	ano: string | number;
	setAno: (value: string | number) => void;
	semestre: string | number;
	setSemestre: (value: string | number) => void;
	fase: string | number;
	setFase: (value: string | number) => void;
	cursos: unknown[];
	habilitarCurso?: boolean;
	habilitarAno?: boolean;
	habilitarSemestre?: boolean;
	habilitarFase?: boolean;
	mostrarTodosCursos?: boolean;
	habilitarFiltroTodasFases?: boolean;
	habilitarFiltroOrientacao?: boolean;
	habilitarFiltroProjeto?: boolean;
	habilitarFiltroTcc?: boolean;
	loading?: boolean;
	anosDisponiveis?: (string | number)[];
	semestresDisponiveis?: (string | number)[];
	fasesDisponiveis?: (string | number)[];
}
...
export default function FiltrosPesquisa({
	...
	const cursosOptions = cursos as CursoOption[];
		...
		<Stack direction="row" spacing={2} alignItems="center">
			{habilitarCurso && (
				<FormControl fullWidth size="small" disabled={loading}>
					...
					<Select value={cursoSelecionado || ""} label="Curso" onChange={handleCursoChange}>
						{mostrarTodosCursos && (
							<MenuItem value="">
