...
export default function AvaliarDefesasOrientador() {
	...
	const [cardAtaPendente, setCardAtaPendente] = useState<CardAtaPendente | null>(null);

	function abrirDialogAta(idTcc: number, fase: number | string | undefined) {
	...
	async function confirmarGerarAta() {
		...
		fecharDialogAta();
		await handleGerarAta(
			cardAtaPendente.idTcc,
			Number(cardAtaPendente.fase),
			localDefesa,
		);
	}
											...
											{editandoTcc[card.chaveUnica] ? (
												<Stack direction="row" spacing={1}>
													...
													<Button
														...
														color="error"
														onClick={() =>
															cancelarEdicao(card.chaveUnica)
														}
													>
														Cancelar
													</Button>
