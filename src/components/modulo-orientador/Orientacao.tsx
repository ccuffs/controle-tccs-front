					...
					>
						...
						<DialogContent>
							<Stack spacing={3} sx={{ mt: 1 }}>
								<Typography variant="body2" color="text.secondary">
									Selecione um arquivo PDF de lista de presença para
									importar discentes automaticamente. O arquivo deve
									conter dados no formato: NOME seguido da MATRÍCULA. Os
									discentes serão vinculados ao curso, ano/semestre e
									fase selecionados abaixo.
								</Typography>

								{/* Filtros para Curso, Ano/Semestre e Fase */}
								<FiltrosPesquisa
									cursoSelecionado={modalCurso ? modalCurso.id : ""}
									setCursoSelecionado={(valor: string | number) => {
										const curso = cursosOptions.find(
											(c) => c.id === valor,
										);
														...
														.map((detalhe, index) => (
															...
															>
																...
																<Chip
																	...
																	size="small"
																	color={
																		orientacaoController.obterCorStatusUpload(
																			detalhe.status,
																		) as
																			| "default"
																			| "success"
						...
						<DialogActions>
							...
							{uploadFile && !uploading && !uploadResults && (
								<Button
									...
									startIcon={<CloudUploadIcon />}
									disabled={
										!modalCurso || !modalAno || !modalSemestre || !modalFase
									}
								>
									Processar PDF
								</Button>
						...
						{loadingEdit ? (
							...
							<Stack spacing={3} sx={{ mt: 1 }}>
													...
													>
														...
														>
															...
															{orientacaoController
																...
																.map((item) => (
																	<MenuItem
																		key={item.docente?.codigo}
																		value={item.docente?.codigo}
																	>
																		{item.docente?.nome}
								...
								<TextField
									...
									value={editData.tema}
									onChange={(e) =>
										handleEditDataChange("tema", e.target.value)
									}
									multiline
									rows={2}
								...
								<TextField
									...
									onChange={(e) =>
										handleEditDataChange("titulo", e.target.value)
									}
									multiline
									rows={2}
								...
								<TextField
									...
									onChange={(e) =>
										handleEditDataChange("resumo", e.target.value)
									}
									multiline
									rows={4}
