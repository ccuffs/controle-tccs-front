...
import type { GerenciarDisponibilidadeHandle } from "../../hooks/useModuloOrientador";

type RowDataGrid = ReturnType<
	typeof disponibilidadeBancaController.gerarRowsDataGrid
>[number];

const GerenciarDisponibilidadeBanca = forwardRef<GerenciarDisponibilidadeHandle>(
	(_props, ref) => {
		const {
			// Estados de filtros
			cursos,
		...
		const generateColumns = (): GridColDef<RowDataGrid>[] => {
			if (!grade || !grade.datas) return [];

			const baseColumns: GridColDef<RowDataGrid>[] = [
				{
					field: "horario",
					headerName: "Horário",
					width: 77,
			...
			const dataColumns: GridColDef<RowDataGrid>[] = datasVisiveis.map((data) => {
				const [anoData, mes, dia] = data.split("-").map(Number);
				...
				return {
					...
					renderHeader: () => (
						...
						>
							<span
								style={{
									fontSize: isFimDeSemana ? "0.55rem" : "0.66rem",
									opacity: 0.85,
								}}
							>
							...
							<span
								style={{
									fontWeight: "bold",
									fontSize: isFimDeSemana ? "0.66rem" : "0.77rem",
								}}
							>
					...
					renderCell: (params) => {
						...
						const isBlocked = disponibilidadeBancaController.isSlotBloqueado(
							bloqueados,
							key,
						);

						if (isBlocked) {
							const bloqueioInfo = disponibilidadeBancaController.tipoBloqueio(
								bloqueados,
								key,
							);
							const nomeDiscente = bloqueioInfo?.nomeDiscente || null;
								...
								<Tooltip title="Próximas datas">
									<span>
										<IconButton
											size="small"
											onClick={() => setPaginaDatas((p) => p + 1)}
											disabled={paginaDatas >= totalPaginas - 1}
										>
											<ChevronRightIcon />
										</IconButton>
									</span>
