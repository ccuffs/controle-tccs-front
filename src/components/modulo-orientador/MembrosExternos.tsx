...
export default function MembrosExternos() {
	...
	const tccsUnicos = useMemo(() => {
		const mapa = new Map<number, TrabalhoConclusao>();
		orientacoes.forEach((o) => {
			const tcc = o.trabalhoConclusao;
			if (tcc && !mapa.has(tcc.id)) {
				mapa.set(tcc.id, tcc);
			}
		});
			...
			<Box>
				<Stack spacing={3}>
					...
					<FormControl size="small" sx={{ maxWidth: 320 }}>
						...
						>
							{cursos.map((c) => (
								<MenuItem key={(c as { id: number }).id} value={(c as { id: number }).id}>
											...
											<CardContent>
												...
												>
													<Stack spacing={0.5}>
														...
														{(m as unknown as { data_defesa?: string }).data_defesa && (
															<Typography variant="body2" color="text.secondary">
																<strong>Defesa:</strong>{" "}
																{(() => {
																	const { dataStr, horaStr } = formatarDataDefesaUTC(
																		(m as unknown as { data_defesa?: string }).data_defesa,
																	);
													...
													<Tooltip title="Remover membro externo">
														<span>
															<IconButton
																color="error"
																onClick={() =>
																	handleRemoverMembro(m.codigo, (m as unknown as { fase: number }).fase)
																}
