...
import {
	...
	type TemaComVagas,
	type TemaGridRow,
} from "../hooks/useTemasDataGrid";

import CustomDataGrid from "./customs/CustomDataGrid";

interface TemasDataGridProps {
	temas: TemaComVagas[];
	onOpenVagasModal?: (row: TemaGridRow) => void;
	onToggleAtivo?: (row: TemaGridRow) => void;
	onDelete?: (row: TemaGridRow) => void;
	isDiscenteView?: boolean;
...
export default function TemasDataGrid({
	...
	const columns: GridColDef<TemaGridRow>[] = [
		{
			field: "docenteNome",
			...
			renderCell: (params) => (
				...
				>
					<Stack direction="row" spacing={1}>
						...
						<Button size="small" color="secondary" onClick={() => onDelete?.(params.row)}>
							Remover
						</Button>
					</Stack>
				</PermissionContext>
