...
import type { TemaComVagas } from "../../hooks/useTemasDataGrid";
import type { Curso } from "../../types/curso";

interface VisualizarTemasTCCProps {
	onAvancarEtapa?: (etapa: number) => void;
}

export default function VisualizarTemasTCC({ onAvancarEtapa }: VisualizarTemasTCCProps) {
	const theme = useTheme();
	const { usuario } = useContext(AuthContext) ?? {};
	const [temas, setTemas] = useState<TemaComVagas[]>([]);
	const [temasAtivos, setTemasAtivos] = useState<TemaComVagas[]>([]);
	const [cursoDiscente, setCursoDiscente] = useState<Curso | null>(null);
	...
	async function getCursoDiscente() {
		try {
			...
			const response = await axiosInstance.get<{ cursos?: Curso[] }>(
				`/usuarios/${usuario?.id}/cursos`,
			);
			if (response.cursos && response.cursos.length > 0) {
				setCursoDiscente(response.cursos[0] ?? null); // Assume o primeiro curso
			} else {
	...
	async function getTemasPorCurso(idCurso: number) {
		setLoading(true);
		try {
			const response = await axiosInstance.get<TemaComVagas[]>(
		...
		<Box>
			<Stack spacing={2}>
				...
				<Typography variant="body2" color="text.secondary">
					...
					{
						Object.keys(
							temasAtivos.reduce<Record<string, boolean>>((acc, tema) => {
								const codigo =
